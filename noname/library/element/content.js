...
function swapStartHand(player, event, { broadcast = false } = {}) {
...
const Content = {
  ...
  async chooseToPlayBeatmap(event, trigger, player) {
    const getBeatmapBroadcastData = (beatmap2) => ({
      name: beatmap2.name,
      filename: beatmap2.filename,
      timeleap: beatmap2.timeleap,
      current: beatmap2.current ?? 0,
      speed: beatmap2.speed ?? 25
    });
    ...
    game.broadcastAll(
      (player2, id, beatmap2) => {
        if (_status.connectMode && beatmap2.timeleap?.length) {
    ...
    event.result = result;
    return;
    async function simulateBeatmapResult(beatmap2) {
      const songDuration = beatmap2.timeleap[beatmap2.timeleap.length - 1] + beatmap2.speed * 100 + 1e3 + (beatmap2.current || 0);
      ...
      if (!_status.connectMode) {
        const skip = () => {
          Array.from(ui.window.getElementsByTagName("audio")).forEach((audio) => {
            if (audio.currentSrc.includes(beatmap2.filename.startsWith("ext:") ? beatmap2.name : beatmap2.filename)) {
              audio.remove();
            }
          });
          control.abort();
      ...
      const acc = Math.min(Math.max(get.rand.apply(get, beatmap2.aiAcc || [mean - half_standard_deviation - get.rand(0, half_standard_deviation), mean + half_standard_deviation + get.rand(0, half_standard_deviation)]), 0), 100);
      let rank;
      if (acc == 100) {
        rank = ["SS", "metal"];
      } else if (acc >= 94) {
...
card.${cardPackName}.${itemName}.${termName}`, term);
  ...
  async replaceHandcardsOL(event, trigger, player) {
    ...
    while (remaining !== 0 && event.activePlayers.length) {
      ...
      const accepted = decisions.filter((current) => !!current);
  ...
  async showCards(event, trigger, player) {
    ...
    if (!flashAnimation) {
      ...
      const createDialog = (cards22, id, customButton2, pileTop2) => {
        const dialog = get.idDialog(id);
        ...
        if (Array.isArray(pileTop2)) {
          for (const button of dialog.buttons) {
            if (pileTop2.includes(button.link)) {
      ...
      game.broadcast(
        (func, cards22, id, customButton2, pileTop2) => {
          func(cards22, id, customButton2, pileTop2);
        },
        createDialog,
        event.hiddencards,
