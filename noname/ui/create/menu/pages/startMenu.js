  ...
  if (connectMenu) {
    ...
    if (!lib.__roomConfigConnectMenuHook) {
      ...
      game.roomConfig.on("configApplied", function() {
        ...
        var active2 = start.querySelector(".active");
        if (active2 && active2.mode && typeof active2._initLink === "function") {
          active2._initLink();
        }
        for (var i2 = 0; i2 < menuUpdates.length; i2++) {
          menuUpdates[i2]();
        }
      });
    }
