  ...
  return {
    ...
    start: [
      async (event, trigger, player) => {
        ...
        event.replacePile = () => {
          const list2 = ["shengdong", "qijia", "caomu", "jinchan", "zengbin", "fulei", "qibaodao", "zhungangshuo", "lanyinjia"];
          const map = {
            shunshou: "shengdong",
            jiedao: "qijia",
            bingliang: "caomu",
          ...
          lib.card.list = lib.card.list.filter((cardInfo) => list2.includes(cardInfo[2]));
        ...
        if (_status.connectMode) {
          ...
          if (_status.mode == "zhong") {
            ...
            if (lib.configOL.enable_commoner || lib.configOL.double_nei) {
              ...
              for (const list2 of lib.config.mode_config.identity.identity.slice(1)) {
                let toReplace;
                if (list2.filter((role) => role == "nei").length >= 2) {
                  toReplace = "nei";
                } else if (list2.filter((role) => role == "zhong").length > list2.filter((role) => role == "fan").length / 2) {
                  toReplace = "zhong";
                } else {
                  toReplace = "fan";
                }
                list2.remove(toReplace);
                list2.push(identity2);
              }
    ...
    game: {
      ...
      chooseCharacterPurpleOL() {
        ...
        const contents = [
          ...
          // step 1
          async (event, trigger, player) => {
            const list2 = ["rZhu", "rZhong", "rNei", "rYe"];
            const list22 = ["bZhu", "bZhong", "bNei", "bYe"];
            list2.randomSort();
            list22.randomSort();
            const identityList = list2.concat(list22);
          ...
          async (event, trigger, player) => {
            ...
            list2.sort((a, b) => lib.group.indexOf(a) - lib.group.indexOf(b));
            event.list = list2;
            event.map = map;
            event.map_zhu = map_zhu;
            let result = await game.bZhu.chooseControl(list2).set("prompt", "请选择冷方武将势力").set("ai", function() {
          ...
          async (event, trigger, player) => {
            ...
            for (const current of players) {
              ...
              var str = "选择角色";
              var list22 = event.map[group].randomGets(4);
              if (event.map_zhu[group]) {
                list22.addArray(event.map_zhu[group].randomGets(2));
              }
              event.map[current.playerid] = list22;
              game.setOLChoicePool(current.playerid, list22);
              list2.push([current, [str, [list22, "character"]], true]);
            }
            game.me.chooseButtonOL(list2, (player2, result) => {
              if (game.online || player2 === game.me) {
                game.initPlayerFromOLResult(player2, result);
          ...
          async (event, trigger, player, result) => {
            ...
            for (const current of players) {
              ...
              var str = "选择角色";
              var list22 = event.map[group].randomRemove(event.mapNum[group]);
              event.map[current.playerid] = list22;
              game.setOLChoicePool(current.playerid, list22);
              list2.push([current, [str, [list22, "character"]], true]);
            }
            game.me.chooseButtonOL(list2, (player2, result2) => {
              if (game.online || player2 === game.me) {
                game.initPlayerFromOLResult(player2, result2);
              }
            });
          },
          // step 5
        ...
        const contents = [
          ...
          async (event, trigger, player) => {
            ...
            for (const [current, identity2] of Iterator.zip([players, identityList])) {
              current.node.identity.classList.add("guessing");
              current.identity = identity2;
              current.setIdentity(list2.includes(identity2) ? "cai2" : "cai");
              if (identity2 === "rZhu" || identity2 === "bZhu") {
                game[identity2] = current;
                current.setIdentity(identity2);
                current.identityShown = true;
          ...
          async (event, trigger, player) => {
            if (game.me === game.rZhu || game.me === game.bZhu) {
              event.isZhu = true;
              const list2 = event.map[event[game.me.identity]].randomGets(4);
              if (event.map_zhu[event[game.me.identity]]) {
                list2.addArray(event.map_zhu[event[game.me.identity]].randomGets(2));
              }
              game.me.chooseButton(true, ["请选择您的武将牌", [list2, "character"]]);
            }
          },
          // step 4
          async (event, trigger, player, result) => {
            ...
            if (!game.rZhu.name) {
              const list2 = event.map[event.rZhu].randomGets(3);
              if (event.map_zhu[event.rZhu]) {
                list2.addArray(event.map_zhu[event.rZhu]);
              }
              const character = list2.randomGet();
              event.map[event.rZhu].remove(character);
              game.rZhu.init(character);
            }
            if (!game.bZhu.name) {
              const list2 = event.map[event.bZhu].randomGets(4);
              if (event.map_zhu[event.bZhu]) {
                list2.addArray(event.map_zhu[event.bZhu].randomGets(2));
              }
              const character = list2.randomGet();
      ...
      chooseCharacterStratagemOL() {
        ...
        const contents = [
          ...
          async (event, trigger, player) => {
            ...
            game.broadcast((list2) => {
              for (const name2 in lib.characterReplace) {
                const names = lib.characterReplace[name2];
                for (let j = 0; j < names.length; ++j) {
                  if (!list2.includes(names[j])) {
                    names.splice(j--, 1);
                  }
                }
              }
          ...
          async (event, trigger, player) => {
            ...
            for (const current of game.players) {
              ...
              const str = "选择角色";
              const sublist = event.list.randomRemove(Math.min(num, num2));
              game.setOLChoicePool(current.playerid, sublist);
              list2.push([current, [str, [sublist, "characterx"]], selectButton, true]);
            }
            const result = await game.me.chooseButtonOL(list2, function(player2, result2) {
              if (game.online || player2 == game.me) {
                game.initPlayerFromOLResult(player2, result2);
              }
            }).forResult();
      ...
      chooseCharacter() {
        ...
        next.addPlayer = (player) => {
          const list2 = get.identityList(game.players.length - 1);
          const list22 = get.identityList(game.players.length);
          for (const item of list2) {
            list22.remove(item);
          }
          player.identity = list22[0];
          player.setIdentity("cai");
        };
        next.removePlayer = () => {
          return game.players.randomGet(game.me, game.zhu);
        };
        next.ai = (player, list2, list22, back) => {
          if (_status.brawl && _status.brawl.chooseCharacterAi && _status.brawl.chooseCharacterAi(player, list2, list22, back) !== false) {
            return;
          }
          const stratagemMode = _status.event.stratagemMode;
          if (_status.event.zhongmode) {
            const listc = list2.slice(0, 2);
            ...
            let choice = list2[0];
            let choice2 = list2[1];
            if (!_status.event.zhongmode && Math.random() - 0.8 < 0 && list22.length) {
              choice = list22[0];
              choice2 = list2[0];
              if (choice2 == choice) {
                choice2 = list2[1];
          ...
          if (back) {
            list2.remove(get.sourceCharacter(player.name1));
            list2.remove(get.sourceCharacter(player.name2));
        ...
        const contents = [
          ...
          async (event, trigger, player) => {
            ...
            for (const name2 in lib.characterReplace) {
              ...
              (hasZhugong ? list22 : list3).push(name2);
            }
            ...
            const getZhuList = () => {
              if (stratagemMode) {
                list22.sort(lib.sort.character);
                return list22;
              }
              const limitZhu = get.config("limit_zhu");
              if (!limitZhu || limitZhu === "off") {
                return list22.slice(0).sort(lib.sort.character);
              }
              if (limitZhu !== "group") {
                const zhuCount = parseInt(limitZhu) || 6;
                return list22.randomGets(zhuCount).sort(lib.sort.character);
              }
              ...
              const list2Random = list22.slice(0);
              const list2x = [];
            ...
            if (stratagemMode) {
              ...
              if (_status.brawl && _status.brawl.chooseCharacter) {
                list2 = _status.brawl.chooseCharacter(event.list, num);
                if (list2 === false || list2 === "nozhu") {
                  list2 = event.list.slice(0, num);
                }
              } else {
                list2 = event.list.slice(0, num);
              ...
              if (_status.brawl && _status.brawl.chooseCharacter) {
                list2 = _status.brawl.chooseCharacter(getZhuList(), list3, num);
                if (list2 === false) {
                  if (event.zhongmode) {
                    list2 = list3.slice(0, 6);
                  } else {
                    list2 = getZhuList().concat(list3.slice(0, num));
                  }
                } else if (list2 === "nozhu") {
                  list2 = event.list.slice(0, num);
                }
            ...
            if (event.swapnodialog) {
              dialog = ui.dialog;
              event.swapnodialog(dialog, list2);
              ...
              dialog = ui.create.dialog(str, "hidden", [list2, "characterx"]);
        ...
        const contents = [
          ...
          async (event, trigger, player) => {
            ...
            if (event.zhongmode) {
              ...
              list2 = getZhuList(list22).concat(list3.randomGets(lib.configOL.choice_zhu));
            }
            game.setOLChoicePool(game.zhu.playerid, list2);
            const chooseButtonEvent = game.zhu.chooseButton(true);
            chooseButtonEvent.set("selectButton", lib.configOL.double_character ? 2 : 1);
            chooseButtonEvent.set("createDialog", ["选择角色", [list2, "characterx"]]);
            chooseButtonEvent.set("onfree", true);
          ...
          async (event, trigger, player) => {
            ...
            for (const currentPlayer of game.players) {
              ...
              const sublist = event.list.randomRemove(Math.min(num, num2));
              game.setOLChoicePool(currentPlayer.playerid, sublist);
              list2.push([currentPlayer, [str, [sublist, "characterx"]], selectButton, true]);
            }
            game.me.chooseButtonOL(list2, (player2, result) => {
              if (game.online || player2 === game.me) {
                game.initPlayerFromOLResult(player2, result);
