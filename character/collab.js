...
const skills = {
  ...
  natulie: {
    ...
    async content(event, trigger, player2) {
      ...
      if (player2.countCards("h") < num) {
        ...
        const result = await player2.chooseToDiscard({ forced: true, position: "h", selectCard: player2.countCards("h") - num }).forResult();
  ...
  natuxing: {
    ...
    async content(event, trigger, player2) {
      if (trigger.name == "useCard") {
        ...
        if (cards2.length) {
          await player2.gain({ cards: cards2, animate: "draw", gaintag: [event.name] });
        }
      }
    },
    mod: {
  ...
  natuyi: {
    ...
    filterTarget(card2, player2, target) {
      return target.hasCards("he") && target != player2;
    },
    selectTarget: [1, 2],
    multiline: true,
    multitarget: true,
