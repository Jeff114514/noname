...
const skills = {
  ...
  clanfenjian: {
    ...
    filterTarget(card, player2, target) {
      return !player2.getStorage("clanfenjian_used").includes(Math.sign(target.getAttackRange() - player2.getAttackRange())) && lib.filter.targetEnabled2(get.autoViewAs(get.info("clanfenjian").viewAs), player2, target);
    },
    viewAs: {
      name: "sha",
      isCard: true,
    ...
    async precontent(event2, trigger, player2) {
      ...
      player2.logSkill(name, event2.result.targets);
      const {
        targets: [target],
        cards: cards2
      } = event2.result;
