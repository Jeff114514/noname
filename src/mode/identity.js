				...
				const contents = [
					...
					async (event, trigger, player) => {
						...
						for (const current of players) {
							...
							game.setOLChoicePool(current.playerid, list2);
				...
				const contents = [
					...
					async (event, trigger, player) => {
						...
						for (const current of game.players) {
							...
							list.push([current, [str, [sublist, "characterx"]], selectButton, true]);
						}
				...
				const contents = [
					...
					async (event, trigger, player) => {
						...
						for (const currentPlayer of game.players) {
							...
							list.push([currentPlayer, [str, [sublist, "characterx"]], selectButton, true]);
						}
						game.me.chooseButtonOL(list, (player, result) => {
							if (game.online || player === game.me) {
								game.initPlayerFromOLResult(player, result);
